 ...
 */
export function serializeThread(thread: CommentThread): CommentThread {
  if (!thread.resolution) return { ...thread }
  return { ...thread, resolution: { ...thread.resolution, element: null } }
}

export function serializeThreads(threads: readonly CommentThread[]): CommentThread[] {
  return threads.map(serializeThread)
}

export function reviveThread(thread: CommentThread): CommentThread {
